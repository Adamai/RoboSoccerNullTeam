		...
		while (true) {
			...
			case PLAY_ON:
				...
				case DEFENSIVA:
					...
					double Ygoal = ballY;
					if (ballPos.getY() > 7) { // ACOMPANHAR Y DA BOLA EM RELAﾇﾃO AO GOL
						Ygoal = 3.4;
				...
				case PASSAR:
					...
					commander.doCatchBlocking(0);
					// 6 ta marcado? Se n縊, chuta pra ele
...
//						dash(ballPos);
//					}
//					// se a bola est� do outro lado do campo e na posse do time
...
//						if (side == EFieldSide.LEFT)
				...
				case STANDBY:
					if (pTemp == selfPerc) {
						//if (side == EFieldSide.LEFT)
						//	System.out.println(Integer.toString(selfPerc.getUniformNumber()) + ": vou atr疽 da bola");
				...
				case OFENSIVA:
					if (isPointsAreClose(selfPerc.getPosition(), ballPos, 0.5)) {
						if (isPointsAreClose(ballPos, goalPos, 30)) {
							...
							if(side == EFieldSide.LEFT) {
								System.out.println("� pra driblar ou passar?");
						}
							for (int i = 1; i < 8; i++) {
								if (fieldPerc.getTeamPlayer(side2, i).getPosition()
										.distanceTo(selfPerc.getPosition()) < marcDist) {// se entrar aqui � pq ta marcado
									if(side == EFieldSide.LEFT) {
										System.out.println("ta marcado");
								}
									for (int j = 1; j < 8; j++) { // verificando se o 7 ta BLOQUEADO tamb駑
							...
							if(side == EFieldSide.LEFT) {
								System.out.println("� pra driblar ou passar 2?");
						}
							for (int i = 1; i < 8; i++) {
								if (fieldPerc.getTeamPlayer(side2, i).getPosition()
									...
									for (int j = 1; j < 8; j++) { // verificando se o 6 ta BLOQUEADO
										if (fieldPerc.getTeamPlayer(side2, j).getPosition().distanceTo(
											...
											atacanteState = PlayerState.DRIBLAR; // 6 ta BLOQUEADO (n縊 entra?)
											if(side == EFieldSide.LEFT) {
												System.out.println("driblando");
										}
											break free;
...
//							System.out.println(Integer.toString(selfPerc.getUniformNumber())+": virando suporte. pTemp � "+pTemp.getUniformNumber());
				...
				case RECEBER:
					...
					if (isPointsAreClose(selfPerc.getPosition(), posicao, 0.1)) { // cheguei na posicao destino?
						...
						atacanteState = PlayerState.OFENSIVA;
						break;
					} else { // n縊 cheguei ainda. Vou para la
						dash(posicao);
					}
